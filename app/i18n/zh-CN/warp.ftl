...
settings-agent-providers-auth-login-description = 通过 Codex 或 Copilot 登录并新增一个受管提供商。每个提供商的 Token 会分别安全存储在系统密钥库。
settings-agent-providers-auth-login-codex-title = Codex Auth
settings-agent-providers-auth-login-codex-description = 打开 Codex OAuth 流程并新增一个 Codex 提供商。
settings-agent-providers-auth-login-codex-login = 使用 Codex 登录
settings-agent-providers-auth-login-copilot-title = Copilot Auth
settings-agent-providers-auth-login-copilot-description = 启动 GitHub 设备码登录流程，将授权码复制到剪贴板并打开浏览器，不会自动填入代码。
settings-agent-providers-auth-login-copilot-login = 使用 Copilot 登录
settings-agent-providers-auth-login-managed-secret = 由登录流程管理。Token 存储在系统密钥库。
settings-agent-providers-copilot-login-code-toast = Copilot 授权码已复制到剪贴板：{ $code }。浏览器打开授权页后，请手动粘贴。
settings-agent-providers-copilot-login-failed-toast = Copilot 登录失败：{ $error }
