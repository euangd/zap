...
settings-agent-providers-auth-login-title = Auth login
settings-agent-providers-auth-login-description = Add a managed provider by signing in with Codex or Copilot. Tokens are stored in the system keychain per provider.
settings-agent-providers-auth-login-codex-title = Codex Auth
settings-agent-providers-auth-login-codex-description = Open the Codex OAuth flow and add a Codex provider.
settings-agent-providers-auth-login-codex-login = Log in with Codex
settings-agent-providers-auth-login-copilot-title = Copilot Auth
settings-agent-providers-auth-login-copilot-description = Start a GitHub device login flow, copy the code to the clipboard, and open the browser without auto-filling it.
settings-agent-providers-auth-login-copilot-login = Log in with Copilot
settings-agent-providers-auth-login-managed-secret = Managed by auth login. Tokens are stored in the system keychain.
settings-agent-providers-copilot-login-code-toast = Copilot code copied to clipboard: { $code }. Paste it manually after the browser opens the authorization page.
settings-agent-providers-copilot-login-failed-toast = Copilot login failed: { $error }
